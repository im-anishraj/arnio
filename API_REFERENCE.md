...
* **`to_dict(redact_sample_values: bool = False) -> dict`**: JSON-friendly export of the full report. When `redact_sample_values=True`, each column's `sample_values` are replaced with `"[REDACTED]"` (same list length). Does not redact `top_values`.
* **`to_pandas() -> pd.DataFrame`**: Returns one row per column; includes `top_values` for string columns, not `sample_values`.

#### Privacy

Profiling exports may include raw cell values in `sample_values` and `top_values`.
`to_html()` and notebook rendering show `top_values`. For safer sharing, use
`to_markdown()`, `summary()`, or `to_dict(redact_sample_values=True)` (samples
only ? `top_values` remain unchanged).
...
#### Methods:
* **`to_dict(redact_sample_values: bool = False) -> dict`**: JSON-friendly export for one column. When `redact_sample_values=True`, replaces that column's `sample_values` with `"[REDACTED]"` (same list length). Does not redact `top_values`.
