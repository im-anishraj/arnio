...
#### Pandas Nullable Dtype Compatibility
Arnio safely round-trips pandas modern nullable extension types. The table below describes how specific pandas dtypes are mapped to Arnio's internal types and back.

| Source pandas dtype | Arnio `_DType` | Output pandas dtype | Null Behavior |
| :------------------ | :------------- | :------------------ | :------------ |
| `Int64Dtype()`     | `INT64`        | `Int64`            | `pd.NA` preserved |
| `BooleanDtype()`   | `BOOL`         | `boolean`          | `pd.NA` preserved |
| `StringDtype()`    | `STRING`       | `string`           | `pd.NA` preserved |
| `Float64Dtype()`   | `FLOAT64`     | `float64` (NumPy)   | Converted to `np.nan` |

*Note: Float columns with nulls are cast to the NumPy `float64` base type during `to_pandas()`, using `np.nan` rather than `pd.NA`.*

#### Example: Round-tripping Nullable Extension Types
