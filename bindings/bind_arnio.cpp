...
PYBIND11_MODULE(_arnio_cpp, m) {
  ...
  py::class_<Column>(m, "Column")
      .def(py::init<const std::string &, DType>(), py::arg("name"),
           py::arg("dtype"))
  ...
  py::class_<Frame>(m, "Frame")
      ...
      .def(
          "column_by_index",
          [](const Frame &f, size_t idx) -> const Column & {
            return f.column(idx);
          },
          py::return_value_policy::reference_internal)
      .def(
          "column_by_name",
          [](const Frame &f, const std::string &name) -> const Column & {
  ...
  // --- Cleaning functions ---
  m.def("drop_nulls", &drop_nulls, py::arg("frame"),
  ...
  m.def("drop_duplicates", &drop_duplicates, py::arg("frame"),
        py::arg("subset") = std::nullopt, py::arg("keep") = "first");

  m.def("strip_whitespace", &strip_whitespace, py::arg("frame"),
        py::arg("subset") = std::nullopt);

  m.def("normalize_case", &normalize_case, py::arg("frame"),
        py::arg("subset") = std::nullopt, py::arg("case_type") = "lower");

  m.def("rename_columns", &rename_columns, py::arg("frame"),
        py::arg("mapping"));
