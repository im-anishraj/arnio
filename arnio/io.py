...
def _validate_delimiter(delimiter: object) -> str:  # type: ignore[arg-type]
...
def _validate_usecols(usecols: object) -> list[str]:  # type: ignore[arg-type]
    """Validate usecols parameter."""
    if isinstance(usecols, str):
        raise TypeError("usecols must be a sequence of column names, not a string")

    try:
        cols = list(cast(list[str], usecols))
    except TypeError as e:
        raise TypeError("usecols must be an iterable of strings") from e
...
def _validate_nrows(nrows: object) -> int:  # type: ignore[arg-type]
