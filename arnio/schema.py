        ...
        if field_def.semantic.startswith("custom:"):
            ...
            if pattern is None:
                ...
                if field_def.semantic == "date":
                    ...
                    # Check basic pattern first, then verify against ISO 3166-1 alpha-2.
                    pattern_invalid = non_null[~text.str.fullmatch(pattern, na=False)]
                    matched_mask = text.str.fullmatch(pattern, na=False)
                    matched = non_null[matched_mask]
                    unassigned = matched[~matched.isin(_ISO_ALPHA2)]
                    invalid = pd.concat([pattern_invalid, unassigned])
                else:
