...
class Schema:
    ...
    def to_yaml(self, path: str | None = None) -> str:
        """Serialize the schema to a stable YAML string (a data contract).

        The output is deterministic: fields are sorted alphabetically, and all
        attributes are written in a fixed order so the file is safe to commit
        to version control and diff in pull requests.

        Parameters
        ----------
        path : str, optional
            When provided, the YAML string is also written to this file path.
            The file is created or overwritten. If the parent directory does
            not exist, a ``FileNotFoundError`` is raised.

        Returns
        -------
        str
            Stable YAML representation of the schema.

        Raises
        ------
        ValueError
            If the schema has callable ``rules`` attached, which cannot be
            serialized.
        ...
        fields:
          email:
            allowed: null
            ...
        >>> schema.to_yaml("contracts/users.schema.yaml")
        """
        if self.rules:
            raise ValueError(
                "Schema rules are not YAML serializable. "
                "Serialize only fields/strict/unique for now."
            )
        ...
        payload = {
            "fields": {
                name: _field_to_yaml_dict(field_def)
                for name, field_def in sorted(self.fields.items())
            },
            "strict": self.strict,
            "unique": list(self.unique) if self.unique is not None else None,
        ...
        yaml_str = yaml.dump(payload, default_flow_style=False, sort_keys=True, allow_unicode=True)

        if path is not None:
            import pathlib
            pathlib.Path(path).write_text(yaml_str, encoding="utf-8")

        return yaml_str

    @classmethod
    def from_yaml(cls, value: str) -> "Schema":
        """Deserialize a schema from a YAML string produced by ``to_yaml()``.

        Parameters
        ----------
        value : str
            YAML string (as returned by ``to_yaml()``).
        ...
        ------
        ValueError
            If the YAML is invalid or the structure is unexpected.
        TypeError
            If required fields have the wrong type.

        Examples
        --------
        >>> yaml_str = schema.to_yaml()
        >>> restored = ar.Schema.from_yaml(yaml_str)
        """
        try:
            import yaml  # type: ignore[import]
        except ImportError as exc:  # pragma: no cover
        ...
        if not isinstance(payload, dict):
            raise TypeError(
                "Schema YAML must decode to an object with 'fields', 'strict', and optional 'unique'."
            )

        fields_payload = payload.get("fields")
        if not isinstance(fields_payload, dict):
            raise TypeError(
                "Schema YAML 'fields' must be an object mapping names to field definitions."
            )
        ...
        strict = payload.get("strict", False)
        if not isinstance(strict, bool):
            raise TypeError("Schema YAML 'strict' must be a boolean.")

        unique = payload.get("unique")
        if unique is not None and not isinstance(unique, list):
            raise TypeError("Schema YAML 'unique' must be a list of strings or null.")
...
def _field_to_yaml_dict(field_def: Field) -> dict[str, Any]:
    """Serialize a Field to a plain dict suitable for YAML output.

    Like ``_field_to_json_dict`` but returns datetime bounds as ISO strings
    and ``allowed`` as a sorted list so the YAML output is deterministic.
    """
    data = _field_to_dict(field_def)
    data["severity"] = field_def.severity
    data["datetime_min"] = (
    ...
    # Ensure allowed is a sorted list (not a set) so YAML output is stable
    if isinstance(data.get("allowed"), (set, frozenset)):
        data["allowed"] = sorted(data["allowed"])
    return data
...
def save_schema(schema: Schema, path: str) -> None:
    """Save a schema to a YAML file (a data contract).

    This is a convenience wrapper around ``Schema.to_yaml(path=...)``.
    The file is safe to commit to version control: fields are sorted
    alphabetically and the output is deterministic across runs.
    ...
    ------
    ValueError
        If the schema has callable ``rules`` that cannot be serialized.
    ImportError
    ...
    --------
    >>> schema = ar.Schema({"email": ar.Email(nullable=False)})
    >>> ar.save_schema(schema, "contracts/users.schema.yaml")
    """
    schema.to_yaml(path=path)


def load_schema(path: str) -> Schema:
    """Load a schema from a YAML file produced by :func:`save_schema`.

    This is a convenience wrapper around ``Schema.from_yaml()``.
    ...
    Examples
    --------
    >>> schema = ar.load_schema("contracts/users.schema.yaml")
    >>> result = schema.validate(frame)
    """
    import pathlib

    yaml_str = pathlib.Path(path).read_text(encoding="utf-8")
