...
class TestSniffDelimiter:
    ...
    def test_handles_crlf_line_endings(self, tmp_path):
        """sniff_delimiter correctly handles Windows CRLF line endings."""
        path = tmp_path / "crlf.csv"
        path.write_text("a,b\r\nc,d\r\n")
        result = sniff_delimiter(path)
        assert result == ","

    def test_handles_carriage_return_only_line_ending(self, tmp_path):
        """sniff_delimiter handles old Mac-style CR-only line endings."""
        path = tmp_path / "cr.csv"
        path.write_text("a,b\rc,d\r")
        result = sniff_delimiter(path)
        assert result == ","

    def test_handles_whitespace_before_header(self, tmp_path):
        """sniff_delimiter skips leading whitespace before detecting delimiter."""
        path = tmp_path / "whitespace.csv"
        path.write_text("   a,b,c\nx,y,z\n")
        result = sniff_delimiter(path)
        assert result == ","

    def test_whitespace_only_with_delimiter_still_detects(self, tmp_path):
        """sniff_delimiter still detects delimiter even in mostly whitespace content."""
        path = tmp_path / "whitespace_only.csv"
        path.write_text("   \n   \t   \n")
        result = sniff_delimiter(path)
        assert result == "\t"

    def test_only_newlines_and_spaces_no_delimiter(self, tmp_path):
        """sniff_delimiter raises when file has only newlines and spaces."""
        path = tmp_path / "no_delimiter.csv"
        path.write_text("   \n  \n   \n")
        with pytest.raises(ValueError, match="Could not determine CSV delimiter"):
            sniff_delimiter(path)

    def test_single_field_no_delimiter(self, tmp_path):
        """sniff_delimiter raises when file has no detectable delimiter."""
        path = tmp_path / "single_field.csv"
        path.write_text("field1\nfield2\nfield3\n")
        with pytest.raises(ValueError, match="Could not determine CSV delimiter"):
            sniff_delimiter(path)
