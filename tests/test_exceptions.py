...
def test_pipeline_step_error_preserves_step_name_and_original_error():
    orig_err = ValueError("bad column list")
    ...
    assert "normalize_names" in str(step_err)
    assert "bad column list" in str(step_err)
...
def test_pipeline_step_error_has_clear_string_representation():
    ...
    assert message
    assert "Error occurred during custom pipeline step 'normalize_names':" in message
