...
class TestToArrowEdgeCases:
    ...
    def test_zero_column_frame(self):
        ...
        frame = from_pandas(pd.DataFrame())
    ...
    def test_all_null_column(self):
        ...
        frame = from_pandas(pd.DataFrame({"val": [None, None, None]}))
        table = to_arrow(frame)
    ...
    def test_single_row_frame(self):
        ...
        frame = from_pandas(pd.DataFrame({"name": ["Alice"], "age": [30]}))
        table = to_arrow(frame)
    ...
    def test_string_dtype_preservation(self):
        ...
        frame = from_pandas(pd.DataFrame({"name": ["Alice", "Bob", "Charlie"]}))
        table = to_arrow(frame)
        assert table.num_columns == 1
    ...
    def test_int_dtype_preservation(self):
        ...
        frame = from_pandas(pd.DataFrame({"count": [1, 2, 3, 4, 5]}))
        table = to_arrow(frame)
        assert table.num_columns == 1
    ...
    def test_float_dtype_with_nan(self):
        ...
        frame = from_pandas(pd.DataFrame({"val": [1.0, float("nan"), 3.0]}))
        table = to_arrow(frame)
        assert table.num_columns == 1
        assert table.num_rows == 3
    ...
    def test_bool_dtype_preservation(self):
        ...
        frame = from_pandas(pd.DataFrame({"active": [True, False, True]}))
        table = to_arrow(frame)
        assert table.num_columns == 1
        assert table.num_rows == 3
    ...
    def test_empty_string_values(self):
        ...
        frame = from_pandas(pd.DataFrame({"name": ["Alice", "", "Charlie"]}))
        table = to_arrow(frame)
        assert table.num_columns == 1
        assert table.num_rows == 3
    ...
    def test_negative_numbers(self):
        ...
        frame = from_pandas(pd.DataFrame({"val": [-10, -5, 0, 5, 10]}))
        table = to_arrow(frame)
        assert table.num_columns == 1
        assert table.num_rows == 5
    ...
    def test_large_integers(self):
        ...
        frame = from_pandas(pd.DataFrame({"val": [10**9, 10**10, 10**11]}))
        table = to_arrow(frame)
        assert table.num_columns == 1
        assert table.num_rows == 3
