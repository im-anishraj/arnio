...
def test_write_json_dict(sample_frame: ArFrame, tmp_path: pathlib.Path) -> None:
    output_file = tmp_path / "output_dict.json"
    ar.write_json(sample_frame, output_file, orient="dict")
    ...
    assert data["id"] == {0: 1, 1: 2, 2: 3}
    assert data["name"] == {0: "Alice", 1: "Bob", 2: "Charlie"}
    assert data["active"] == {0: True, 1: False, 2: True}
...
def test_write_json_invalid_path(sample_frame: ArFrame) -> None:
    with pytest.raises(TypeError, match="path must be a string, bytes, or os.PathLike object"):
        ar.write_json(sample_frame, 123)  # type: ignore


def test_write_json_unsupported_extension(sample_frame: ArFrame, tmp_path: pathlib.Path) -> None:
    with pytest.raises(ValueError, match="Unsupported file format.*only supports .json"):
        ar.write_json(sample_frame, tmp_path / "output.csv")


def test_write_json_unsupported_orient(sample_frame: ArFrame, tmp_path: pathlib.Path) -> None:
    with pytest.raises(ValueError, match="Unsupported orient"):
        ar.write_json(sample_frame, tmp_path / "out.json", orient="split")


def test_write_json_invalid_indent(sample_frame: ArFrame, tmp_path: pathlib.Path) -> None:
