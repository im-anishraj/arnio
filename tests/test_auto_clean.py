...
class TestAutoCleanEdgeCases:
    def test_auto_clean_entirely_clean_data(self):
        ...
        assert explanation.steps == []  # No steps for clean data
    ...
    def test_auto_clean_extremely_skewed_outliers(self):
        ...
        # Create a dataset where one column is extremely skewed (outliers) and another has mostly nulls
        df = pd.DataFrame(
            {
                "id": [1, 2, 3, 4, 5],
                "skewed_val": [
                    ...
                    10000.0,
                    10.5,
                ],  # 10000.0 is an extreme outlier
                "mostly_null": [None, None, "val", None, None],  # 80% null
            }
        )
        frame = ar.from_pandas(df)
        ...
        if isinstance(result, tuple):
            ...
            # return_report=True always returns a tuple
            cleaned = result
        ...
        # Ensure we can export and inspect it
        res_df = ar.to_pandas(cleaned)
        assert len(res_df) == 5
